...
from six import print_,integer_types,iteritems

from data_structures import Vector,Field,Dimension,DictProxy,TupleProxy, \
    Tensor,SymmTensor,Unparsed,UnparsedList,Codestream,DictRedirection, \
    BinaryBlob,BinaryList,BoolProxy
...
class PlyParser(object):
    ...
    def parse(self,content):
        ...
        return self.yacc.parse(content,debug=debug)
...
#        print size % (t.lexer.binary_listlen), len(t.lexer.lexdata)
    ...
    def p_include(self,p):
        ...
        if self.doMacros:
            fName=os.path.join(self.directory(),p[2][1:-1])
            read=True
            if p[1]=="includeIfPresent" and not os.path.exists(fName):
                read=False
            if read and not os.path.exists(fName):
    ...
    def parseSubst_root(self,nm,stck):
        ...
        for i,di in enumerate(reversed(stck)):
            if nm in di:
                if rest==None:
                    v=DictRedirection(copy.deepcopy(di[nm]),
                                      di[nm],
                                      nm)
                    return v
                else:
    ...
    def p_dictline(self,p):
        ...
        if len(p)==4 and self.inHeader and p[1]=="format" and type(p[2])==str:
            if p[2]=="binary":
                ...
                raise Exception("Don't know how to parse file format",p[0]["format"])
...
class PyFoamParserError(Exception):
    def __init__(self,descr,data=None):
        Exception.__init__(self,"Parser Error:",descr)
        self.descr=descr
        self.data=data
